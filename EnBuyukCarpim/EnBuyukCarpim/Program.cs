...
namespace EnBuyukCarpim
{
    class Program
    {
        ...
        private static void CarpimlariBul() 
        {
            int[] buffer = new int[1000] { 6,4,0,7,7,1,7,6,5,3,1,3,3,0,6,2,4,9,1,9,2,2,5,1,1,9,6,7,4,4,2,6,5,7,4,7,4,2,3,5,5,3,4,9,1,9,4,9,3,4,9,6,9,8,3,5,2,0,3,
...
6,1,8,8,2,6,7,0,4,2,8,2,5,2,4,8,3,6,0,0,8,2,3,2,5,7,5,3,0,4,2,0,7,5,2,9,6,3,4,5,0};
            int carpanSayisi = 2;
            Console.WriteLine("Lütfen carpan sayisini giriniz => ");
            carpanSayisi = int.Parse(Console.ReadLine());
            long carpim = 1;
            int index = 0;
            long[] carpimDizi = new long[buffer.Length - carpanSayisi];
            for (int j = 0; j < buffer.Length - carpanSayisi; j++)
            {                
                for (int i = j; i < carpanSayisi + j; i++)
                {
                    carpim *= buffer[i];
                ...
                if (carpim != 0)
                {
                    Console.WriteLine(carpim + " bulundugu index --> " + index + " bulundugu son index --> " + (index + carpanSayisi));
                }
                index++;
                carpim = 1;
            ...
            Console.WriteLine("carpımların en buyuk degeri --> " + Max(carpimDizi));
            Console.ReadKey();
        }
        private static long Max(long[] sayilar)
        {
           long buyuk = sayilar[0];
